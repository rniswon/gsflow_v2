...
C        AND SPECIFIC YIELD WHEN UNSATURATED FLOW IS ACTIVE.
     ...
     +                   /(DELR(jrch)*DELC(irch))
            ELSEIF ( Iunitupw.GT.0 ) THEN
              THTR(ii) = THTS(ii) - SC2UPW(jrch, irch, krch)
     +                   /(DELR(jrch)*DELC(irch))
...
! RGN 5/8/09 Fixed calculation of THTR to include HUF
     ...
     +                         /(DELR(jrck)*DELC(irck))
                  ...
                  ELSEIF ( Iunitupw.GT.0 ) THEN
                    THTR(irch) = THTS(irch) - SC2UPW(jrck, irck, krck)
     +                           /(DELR(jrck)*DELC(irck))
                  END IF
                END IF
        ...
        ELSEIF ( Iunitupw.GT.0 ) THEN
          IF ( IBOUND(jrck, irck, krck).GT.0 ) THEN
            IF ( LAYVKAUPW(krck).EQ.0 ) THEN
              UHC(ichk) = VKAUPW(jrck, irck, krck)
            ELSE
              UHC(ichk) = VKAUPW(jrck, irck, krck)*
     +                    HKUPW(jrck, irck, krck)
            END IF
          END IF
        ELSE
          IF ( IBOUND(jrck, irck, krck).GT.0 ) THEN
...
C     ARGUMENTS
C     -----------------------------------------------------------------
      INTEGER Kkiter, Kkper, Iunitlak, Igrid, Kkstp, Iunitnwt, Iunitrch,
     +        Iunituzf
C     -----------------------------------------------------------------
C     LOCAL VARIABLES
C     -----------------------------------------------------------------
...
!     +                        HNEW, BUFF, BOTM, LBOTM, IUNIT
      USE GLOBAL,       ONLY: NCOL, NROW, NLAY, IOUT, ISSFLG, IBOUND,
     +                        HNEW, BUFF, BOTM, LBOTM, IUNIT !, DELR, DELC, IUNIT
!IFACE
...
Cdep   added compact budget option for streamflow out of reach
     ...
     +              BUFF, NCOL, NROW, NLAY, IOUT)
      ...
      SFRRATIN = RATIN
      SFRRATOUT = RATOUT
! for SNTEMP GW flows by segment
!!      if (issflg==0)then
!!      do l=1,NSEGDIM
!!      WRITE (923, 2) (Nowtime(I),I=1,6),l,seg(27,l)
!!      end do
!!      end if
      
!!    2 FORMAT ('I4, 2('-', I2.2), I3.2, ':', I2.2, I3.2,I10,e15.6)

!924   format(e20.10,5000e20.10)
!
C
C52-----RETURN.
      RETURN
...
C     1                        DVRSFLW, STRIN, STROUT
C      USE GWFLAKMODULE, ONLY: SURFOT
      USE GWFBASMODULE, ONLY: DELT
      IMPLICIT NONE
C     -------------------------------------------------------------------
C     SPECIFICATIONS:
...
C     LOCAL VARIABLES
C     -------------------------------------------------------------------
      INTEGER :: ISTSG, IRNUM, L, ISTSGOLD, REACHNUMINSEG, II !, IRNUM, II
