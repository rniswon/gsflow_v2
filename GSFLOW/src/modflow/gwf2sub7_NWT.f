      MODULE GWFSUBMODULE
        ...
        DOUBLE PRECISION,PARAMETER :: NOCOMV=-1.0D30
...
C16-----ARE CONSISTANT WITH STARTING HEADS.
       ...
       IF(HC(LOC2).LT.BOTM(IC,IR,LBOTM(K))) HC(LOC2)=NOCOMV
  270  CONTINUE
  280  CONTINUE
      ENDIF
      IF(NDF) THEN
...
C18-----READ IN ARRAYS FOR SYSTEMS OF DELAY INTERBEDS.
 ...
 308    FORMAT(/,1X,' SYSTEM',I4,' OF DELAY BEDS:')
  ...
  320   CONTINUE
        ...
        IF(DHC(LOC4).LT.BOTM(IC,IR,LBOTM(K))) DHC(LOC4) = NOCOMV
  325   CONTINUE
  330   CONTINUE
       ENDIF
       WRITE(IOUT,308) KQ
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,    ONLY:HNEW,NCOL,NROW,ISSFLG,BOTM,LBOTM
...
C       LOC2H=NKR+IC
       ...
       IF(HC(LOC2).LT.BOTM(IC,IR,LBOTM(K))) HC(LOC2)=NOCOMV             !seb NO MORE INELASTIC COMPACTION WHEN CELL IS DRY
       IF(HC(LOC2).GT.HTMP) HC(LOC2)=HTMP
   10  CONTINUE
   20  CONTINUE
      ENDIF
...
C3------DETERMINE STORAGE CAPACITIES FOR CELL AT START AND END OF STEP
       ...
       RHO2=RHO1
       HCTMP=HC(LOC2)
       IF(HNEW(IC,IR,K).LT.HCTMP) RHO2=SCV(LOC2)*TLED                   !seb if CELL EVER IS DRY HCTMP=-1E100 SO THIS WILL NEVER BE TRUE
C
...
C15-----UPDATE PRECONSOLIDATION HEAD ARRAY
       ...
       HTMP=HNEW(IC,IR,K)
       IF(HTMP.LT.HC(LOC2)) HC(LOC2)=HTMP
       IF(HC(LOC2).LT.BOTM(IC,IR,LBOTM(K))) HC(LOC2)=NOCOMV             !seb NO MORE INELASTIC COMPACTION WHEN CELL IS DRY
  300  CONTINUE
  310  CONTINUE
      ENDIF
