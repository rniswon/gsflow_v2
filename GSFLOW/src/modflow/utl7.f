...
C          TEST IF OPEN\CLOSE FILE EXISTS
 ...
 1115      FORMAT(1X,/1X,'OPENING BINARY FILE ON UNIT ',I4,':',/1X,A)
  ...
  115      FORMAT(1X,/1X,'OPENING FILE ON UNIT ',I4,':',/1X,A)
           ACCARG='SEQUENTIAL'
           OPEN(UNIT=IN,FILE=FNAME,ACTION=ACTION(1),ACCESS=ACCARG)
         END IF
         ICLOSE=1
         IF (IBINARY.NE.1) READ(IN,'(A)') LINE
      END IF
...
C----------------FORMAT 60I1
  ...
  551 FORMAT(1X,I3,1X,60(1X,I0):/(5X,60(1X,I0)))
      GO TO 510
C
C----------------FORMAT 40I2
...
C6-------PRINT TITLE ON EACH STRIP DEPENDING ON ILAY
    ...
    1    FORMAT('1',/2X,A,' IN LAYER ',I0,' AT END OF TIME STEP ',I0,
     1     ' IN STRESS PERIOD ',I0,/,2X,75('-'))
      ELSE IF(ILAY.LT.0) THEN
         WRITE(IOUT,2) TEXT,KSTP,KPER
    2    FORMAT('1',/1X,A,' FOR CROSS SECTION AT END OF TIME STEP ',I0,
     1     ' IN STRESS PERIOD ',I0,/,1X,79('-'))
      END IF
C
...
C------------ FORMAT 10F6.1
  ...
  141 FORMAT(1X,I3,1X,10(1X,F0.1):/(5X,10(1X,F0.1)))
      GO TO 1000
C
C------------ FORMAT 10F6.2
...
C1------WRITE AN UNFORMATTED RECORD IDENTIFYING DATA.
    ...
    1 FORMAT(1X,'UBUDSV SAVING "',A16,'" ON UNIT',I3,
     1     ' AT TIME STEP ',I0,', STRESS PERIOD ',I0)
