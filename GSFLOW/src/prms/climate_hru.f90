...
! Local Variables
      INTEGER :: yr, mo, dy, i, hr, mn, sec, jj, ierr, missing, ios !, write_tmin_tmax
...
!***********************************************************************
      ...
      IF ( Process_flag==RUN ) THEN
        IF ( Climate_temp_flag==ACTIVE ) THEN
          CALL read_cbh_values('tmaxf', Tmax_unit, Tmaxf)
          CALL read_cbh_values('tminf', Tmin_unit, Tminf)
          Basin_tmax = 0.0D0
          Basin_tmin = 0.0D0
          Basin_temp = 0.0D0
        ENDIF
        ...
        IF ( Climate_precip_flag==ACTIVE ) THEN
          CALL read_cbh_values('hru_ppt', Precip_unit, Hru_ppt)
          IF ( istop==0 ) THEN
        ...
        IF ( Climate_potet_flag==ACTIVE ) THEN
          CALL read_cbh_values('potet', Et_unit, Potet)
          Basin_potet = 0.0D0
        ENDIF

        IF ( AET_cbh_flag==ACTIVE ) THEN
          CALL read_cbh_values('AET_external', AET_unit, AET_external)
          Basin_aet_external = 0.0D0
        ENDIF

        IF ( PET_cbh_flag==ACTIVE ) THEN
          CALL read_cbh_values('PET_external', PET_unit, PET_external)
          IF ( istop==0 ) THEN
...
!                PRINT *, yr, mo, dy, ii, PET_external(ii), AET_external(ii), Ag_frac(ii)
        ...
        IF ( irrigated_area_cbh_flag==ACTIVE ) THEN
          CALL read_cbh_values('irrigated_area', Irrigated_area_unit, Irrigated_area)
        ...
        IF ( Climate_swrad_flag==ACTIVE ) THEN
          ...
          ENDIF
          IF ( ios==-1 ) THEN
            PRINT *, 'ERROR, reading CBH File, variable: ', 'swrad', yr, mo, dy
            PRINT *, '       End-of-File found'
          ENDIF
          IF ( ios/=0 ) THEN
            istop = 1
          ELSE
            IF ( Cbh_check_flag==ACTIVE ) CALL read_cbh_date(yr, mo, dy, 'swrad')
            IF ( istop==0 ) THEN
        ...
        IF ( Climate_transp_flag==ACTIVE ) THEN
          ...
          ENDIF
          IF ( ios==-1 ) THEN
            PRINT *, 'ERROR, reading CBH File, variable: ', 'transp_on', yr, mo, dy
            PRINT *, '       End-of-File found'
          ENDIF
          IF ( ios/=0 ) THEN
            istop = 1
          ELSE
            IF ( Cbh_check_flag==ACTIVE ) CALL read_cbh_date(yr, mo, dy, 'transp_on')
            IF ( istop==0 ) THEN
              IF ( cbh_active_flag==ACTIVE ) THEN
                Transp_on = -999
                DO i = i, Ncbh
        ...
        IF ( Humidity_cbh_flag==ACTIVE ) THEN
          CALL read_cbh_values('humidity_hru', Humidity_unit, Humidity_hru)
          Basin_humidity = 0.0D0
        ENDIF

        IF ( Albedo_cbh_flag==ACTIVE ) CALL read_cbh_values('albedo_hru', Albedo_unit, Albedo_hru)

        IF ( Cloud_cover_cbh_flag==ACTIVE ) CALL read_cbh_values('cloud_cover_cbh', Cloud_cover_unit, Cloud_cover_cbh)

        IF ( Windspeed_cbh_flag==ACTIVE ) THEN
          CALL read_cbh_values('windspeed_hru', Windspeed_unit, Windspeed_hru)
...
!   Declared Parameters
     ...
     &         'none')/=0 ) CALL read_error(1, 'cbh_hru_id')
      ...
      ELSEIF ( Process_flag==INIT ) THEN
        ...
        IF ( Climate_temp_flag==ACTIVE ) THEN
          ...
          CALL find_cbh_header_end(Tmax_unit, Tmax_day, 'tmax_day', 1)
          IF ( istop==0 ) THEN
            ...
            ENDIF
          ENDIF
          CALL find_cbh_header_end(Tmin_unit, Tmin_day, 'tmin_day', 1)
          IF ( istop==0 ) THEN
     ...
     &         CALL read_error(2, 'potet_cbh_adj')
        ...
        IF ( AET_cbh_flag==ACTIVE ) THEN
          ...
          CALL find_cbh_header_end(AET_unit, AET_cbh_file, 'AET_cbh_file', 1)
          IF ( istop==0 ) THEN
        ...
        IF ( PET_cbh_flag==ACTIVE ) THEN
          ...
          CALL find_cbh_header_end(PET_unit, PET_cbh_file, 'PET_cbh_file', 1)
          IF ( istop==0 ) THEN
        ...
        IF ( irrigated_area_cbh_flag==ACTIVE ) THEN
          ...
          CALL find_cbh_header_end(irrigated_area_unit, irrigated_area_cbh_file, 'irrigated_area_cbh_file', 1 )
          IF ( istop==0 ) THEN
        ...
        IF ( Climate_transp_flag==ACTIVE ) THEN
          ...
          CALL find_cbh_header_end(Transp_unit, Transp_day, 'transp_day', 1)
          IF ( istop==0 ) THEN
        ...
        IF ( Climate_swrad_flag==ACTIVE ) THEN
          ...
          CALL find_cbh_header_end(Swrad_unit, Swrad_day, 'swrad_day', 1)
          IF ( istop==0 ) THEN
        ...
        IF ( Humidity_cbh_flag==ACTIVE ) THEN
          ...
          CALL find_cbh_header_end(Humidity_unit, Humidity_day, 'humidity_day', 2)
          IF ( istop==0 ) THEN
        ...
        IF ( Windspeed_cbh_flag==ACTIVE ) THEN
          ...
          CALL find_cbh_header_end(Windspeed_unit, Windspeed_day, 'windspeed_day', 1)
          IF ( istop==0 ) THEN
        ...
        IF ( Albedo_cbh_flag==ACTIVE ) THEN
          ...
          CALL find_cbh_header_end(Albedo_unit, Albedo_day, 'albedo_day', 1)
          IF ( istop==0 ) THEN
        ...
        IF ( Cloud_cover_cbh_flag==ACTIVE ) THEN
          ...
          CALL find_cbh_header_end(Cloud_cover_unit, Cloud_cover_day, 'cloud_cover_day', 1)
          IF ( istop==0 ) THEN
        ...
        IF ( istop==1 ) THEN
          PRINT *, 'ERROR in climate_hru initialize'
          ERROR STOP ERROR_cbh
        ENDIF
...
!***********************************************************************
      SUBROUTINE read_cbh_date(Year, Month, Day, Var)
...
!***********************************************************************
      ...
      IF ( right_day==0 ) THEN
        PRINT *, 'ERROR, reading CBH File, variable: ', Var
...
!     Read CBH values
!***********************************************************************
      SUBROUTINE read_cbh_values(Vartype, CBH_unit, New_values)
...
!***********************************************************************
      ...
      ELSE
        IF ( Cbh_check_flag == ACTIVE ) THEN
          IF ( ios /= 0 ) THEN
            PRINT *, 'ERROR, READING: ', vartype, yr, mo, dy, ios
            istop = 1
          ELSE
...
!***********************************************************************
  subroutine find_cbh_header_end(Iunit, Fname, Paramname, Iflag)
    ...
    if (ios /= 0) then
      if (Iflag == 2) then ! this signals climate_hru to ignore the Humidity CBH file, could add other files
