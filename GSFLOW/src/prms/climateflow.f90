...
!   Declared Parameters - Temp
      ...
      REAL, SAVE, ALLOCATABLE :: Tmax_adj_offset(:, :)
...
!***********************************************************************
     ...
     &    Init_vars_from_file, Temp_flag, Precip_flag, Glacier_flag, Nmonths, &
     ...
     &    AG_flag, PRMS_land_iteration_flag, GSFLOW_flag, gwflow_flag, Nhrucell, activeHRU_inactiveCELL_flag, documentation_files_flag
...
! Solar Radiation variables
     ...
     &     'Langleys', Basin_potsw)
      ...
      IF ( Solrad_flag==ddsolrad_module .OR. Solrad_flag==ccsolrad_module .OR. documentation_files_flag==1 ) THEN
...
! Potential ET Variables
     ...
     &     Et_flag==potet_pm_sta_module .OR. documentation_files_flag==1 ) THEN
     ...
     &       'kilopascals/degrees Celsius', Vp_slope)
        IF ( Et_flag==potet_pm_module .OR. Et_flag==potet_pm_sta_module .OR. documentation_files_flag==1 ) THEN
          ALLOCATE ( Vp_sat(Nhru) )
          CALL declvar_real(Et_module, 'vp_sat', 'nhru', Nhru, &
     &         'Saturation vapor pressure for each HRU', &
     &         'kilopascals', Vp_sat)
        ENDIF
        IF ( (Et_flag==potet_pm_module .OR. Et_flag==potet_pt_module .AND. Humidity_cbh_flag==OFF) .OR. documentation_files_flag==1 ) THEN
...
! Soilzone variables
     ...
     &     'inches', Ssr_to_gw)
      ...
      IF ( activeHRU_inactiveCELL_flag == ACTIVE .OR. documentation_files_flag==1 ) THEN
...
!      CALL declvar _real(Soilzone_module, 'soil_zone_max', 'nhru', Nhru, &
...
!     &     'inches', Soil_zone_max)/=0
     ...
     &     'inches', Basin_soil_to_gw)
      ...
      IF ( GSFLOW_flag==ACTIVE .OR. documentation_files_flag==1 ) THEN
...
! glacier variables
     ...
     &     ALLOCATE ( Psta_elev_meters(Nrain), Psta_elev_feet(Nrain) )
      ...
      ALLOCATE ( Tmax_allrain_c(Nhru,Nmonths) )

! Declare Parameters
      IF ( Temp_flag<climate_hru_module .OR. documentation_files_flag==1 ) THEN
     ...
     &       'elev_units')/=0 ) CALL read_error(1, 'tsta_elev')
      ...
      IF ( Temp_flag==temp_1sta_module .OR. Temp_flag==temp_laps_module .OR. Temp_flag==temp_sta_module .OR. documentation_files_flag==1 ) THEN
     ...
     &     Temp_flag==ide_dist_module .OR. Temp_flag==xyz_dist_module .OR. Temp_flag==temp_sta_module .OR. documentation_files_flag==1 ) THEN
        ALLOCATE ( Tmax_aspect_adjust(Nhru,Nmonths) )
        IF ( bias_adjust_flag==ACTIVE .OR. documentation_files_flag==1 ) THEN
          ALLOCATE ( Tmax_adj_offset(Nhru,Nmonths) )
     ...
     &     'inches')/=0 ) CALL read_error(1, 'imperv_stor_max')
      ...
      IF ( Init_vars_from_file==0 .OR. Init_vars_from_file==2 .OR. Init_vars_from_file==5 .OR. documentation_files_flag==1 ) THEN
        IF ( PRMS4_flag==ACTIVE .OR. documentation_files_flag==1 ) THEN
...
!***********************************************************************
     ...
     &    FEET, FEET2METERS, METERS2FEET, FAHRENHEIT, INACTIVE, LAKE, ddsolrad_module, ccsolrad_module
      ...
      USE PRMS_MODULE, ONLY: Nhru, Nssr, Nevap, Nlake, Ntemp, Nrain, Nsol, Nmonths, &
...
!***********************************************************************
     ...
     &     Temp_flag==ide_dist_module .OR. Temp_flag==xyz_dist_module .OR. Temp_flag==temp_sta_module ) THEN
        IF ( getparam_real(Temp_module, 'tmin_adj', Nhru*Nmonths, Tmin_aspect_adjust)/=0 ) &
                           CALL read_error(2, 'tmin_adj')
        IF ( bias_adjust_flag==ACTIVE ) THEN
          IF ( getparam_real(Temp_module, 'tmax_adj_offset', Nhru*Nmonths, Tmax_adj_offset)/=0 ) &
                        CALL read_error(2, 'tmax_adj_offset')
          Tmax_aspect_adjust = Tmin_aspect_adjust + Tmax_adj_offset
          DEALLOCATE ( Tmax_adj_offset )
        ELSE
          IF ( getparam_real(Temp_module, 'tmax_adj', Nhru*Nmonths, Tmax_aspect_adjust)/=0 ) &
     ...
     &    CALL read_error(2, 'tmax_allsnow')
      ...
      IF ( PRMS4_flag==ACTIVE ) THEN
        IF ( getparam_real(Precip_module, 'tmax_allrain', Nhru*Nmonths, Tmax_allrain)/=0 ) &
     ...
     &                                               ' tmax_allrain set to tmax_allsnow'
        ...
        ENDDO
      ELSE
        IF ( getparam_real(Precip_module, 'tmax_allrain_offset', Nhru*Nmonths, Tmax_allrain_offset)/=0 ) &
     &                     CALL read_error(2, 'tmax_allrain_offset')
      ...
      IF ( getparam_real(Precip_module, 'adjmix_rain', Nhru*Nmonths, Adjmix_rain)/=0 ) CALL read_error(2, 'adjmix_rain')
      ...
      IF ( Solrad_flag==ddsolrad_module .OR. Solrad_flag==ccsolrad_module ) THEN
        ...
        IF ( getparam_real(Solrad_module, 'ppt_rad_adj', Nhru*Nmonths, Ppt_rad_adj)/=0 ) &
     &       CALL read_error(2, 'ppt_rad_adj')
        IF ( getparam_real(Solrad_module, 'radmax', Nhru*Nmonths, Radmax)/=0 ) CALL read_error(2, 'radmax')
      ELSE
        Basin_solsta = 0
        Rad_conv = 1.0
      ENDIF
     ...
     &       CALL checkdim_bounded_limits('hru_pansta', 'nevap', Hru_pansta, Nhru, 0, Nevap, Inputerror_flag)
      ...
      IF ( getparam_real('intcp', 'epan_coef', Nhru*Nmonths, Epan_coef)/=0 ) CALL read_error(2, 'epan_coef')
      ...
      IF ( Et_flag==potet_pt_module .OR. Et_flag==potet_pm_module .OR. Et_flag==potet_pm_sta_module ) THEN
        ...
        IF ( Et_flag==potet_pt_module .OR. Et_flag==potet_pm_module ) THEN
          IF ( Humidity_cbh_flag==OFF ) THEN
            IF ( getparam_real(Et_module, 'humidity_percent', Nhru*Nmonths, Humidity_percent)/=0 ) &
...
!***********************************************************************
    IF ( In_out==SAVE_INIT ) THEN
      IF ( text_restart_flag==OFF ) THEN
        ...
        IF ( gwflow_flag==1 ) WRITE ( Restart_outunit ) Gwres_stor
        IF ( Dprst_flag==ACTIVE ) THEN
