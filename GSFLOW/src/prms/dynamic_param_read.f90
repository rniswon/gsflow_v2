...
!***********************************************************************
     ...
     &      Dyn_springfrost_flag, Dyn_snareathresh_flag, Et_flag, PRMS4_flag, Agriculture_flag, Dyn_ag_flag
        IMPLICIT NONE
        ! Local Variables
        character(len=*), parameter :: MODDESC = 'Time Series Data'
        character(len=*), parameter :: MODNAME = 'dynamic_param_read'
        character(len=*), parameter :: Version_dynamic_param_read = '2020-10-09'
...
! Local Variables
      ...
      REAL :: harea, frac_imperv, tmp, perv_area, dprstfrac, soil_adj, agfrac
...
!***********************************************************************
      ...
      IF ( Imperv_frac_flag==ON .OR. Dprst_frac_flag==ON .OR. Dprst_depth_flag==ON ) THEN
        ...
        IF ( Check_imperv==ON .OR. Check_dprst_frac==ON .OR. check_dprst_depth_flag==ON .OR. Check_ag_frac==ON ) THEN
          ...
          DO i = 1, Nhru
            ...
            IF ( perv_area>0.0 ) THEN
              IF ( Hru_perv(i)/=perv_area ) THEN
                tmp = Hru_perv(i)/perv_area
