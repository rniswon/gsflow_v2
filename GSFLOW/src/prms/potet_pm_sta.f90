...
!***********************************************************************
      MODULE PRMS_POTET_PM_STA
        ...
        character(len=*), parameter :: Version_potet = '2025-02-12'
!        REAL, SAVE, ALLOCATABLE :: Tavgc_ante(:) ! if used later, may need to save to restart file
        ! Declared Parameters
...
!...PSCHOMETRIC CONSTANT AT AVG TEMPERATURE FOR ALTITUDE, KPA:
          ...
          ! atmospheric pressure for altitude, kPa
          ! p_atm = 101.3 - 0.003215*Hru_elev_feet(i) !old
          ! psychrometric constant, kilopascals per degrees C
          ! psycnst = 1006.0*p_atm/(0.622*elh) !old
          ! atmospheric pressure for altitude, kPa
...
! humidity Data File variable is input as percent so divided by 100 to be in units of decimal fraction
          ...
          ! temp = (LOG(Humidity_hru(i)/100.0) + ((17.26939*tavg) / (237.3+tavg))) / 17.26939 !old
!          Tempc_dewpt(i) = (237.3*temp) / (1.0 - temp) !old
...
!          Vp_actual(i) = 0.1*sat_vapor_press_poly(Tempc_dewpt(i)) ! ea, old

!...SATURATION VAPOR PRESSURE AT AVG TEMP, KILOPASCALS (KPA):
! divide by 10 to convert millibar to kpa
!          Vp_sat(i) = 0.1*(sat_vapor_press_poly(Tmaxc(i)) + sat_vapor_press_poly(Tminc(i))) * 0.5 ! es, old
...
!          Vp_slope(i) = (409.8*sat_vapor_press_poly(tavg)) / (tavg+237.3)**2 ! delta, old
...
! there is no divide by zero.
          IF ( .not.(Soltab_potsw(Jday,i) > 10.0) ) THEN
            stab = 10.0
          ELSE
...
! 4.903E-09 = Stefan-Boltzmann constant
      ...
      &                  * (0.34 - 0.14*(Vp_actual(i)**0.5)) * (((1.35*sw) / stab) - 0.35)
          ! net long-wave
!          lwrad = 4.903D-09*(DBLE(tavg)+273.16D0)*(0.34D0-0.14D0*DBLE(SQRT(Vp_actual(i))))& !old
!      &           *(1.35D0*DBLE(Swrad(i)/Soltab_potsw(i,Jday))-0.35D0) ! what is solf Rs/Rso (? Rs = Swrad - Rso = potsw_soltab??) !old
!          Lwrad_net(i) = SNGL(lwrad) ! net radiation = Rns - Rnl; Rns = (1.0-albedo)*swrad !old
!          Net_radiation(i) = Hru_albedo_coef(i)*Swrad(i) - Lwrad_net(i) !old
...
!          heat_flux = -4.2 * (Tavgc_ante(i)-Tavgc(i)) ! could use solrad_tmax or running avg instead of Tavgc_ante !old
!          !heat_flux = 0.0 ! Irmak and others (2012) says equal to zero for daily time step ! G, moved outside loop

!          a = 0.408*Vp_slope(i)*(Lwrad_net(i)/23.88-heat_flux) !old
!          a = 0.408*Vp_slope(i)*(Net_radiation(i)/23.88-heat_flux) !old
...
!******Get parameters
      ELSEIF ( Process_flag==INIT ) THEN
        Vp_sat = 0.0
        IF ( getparam_real(MODNAME, 'pm_n_coef', Nhru*Nmonths, Pm_n_coef)/=0 ) CALL read_error(2, 'pm_n_coef')
        IF ( getparam_real(MODNAME, 'pm_d_coef', Nhru*Nmonths, Pm_d_coef)/=0 ) CALL read_error(2, 'pm_d_coef')
        IF ( getparam_real(MODNAME, 'crop_coef', Nhru*Nmonths, Crop_coef)/=0 ) CALL read_error(2, 'crop_coef')
!        Tavgc_ante = Tavgc
        IF ( getparam_int(MODNAME, 'hru_windspeed_sta', Nhru, Hru_windspeed_sta)/=0 ) &
             CALL read_error(2, 'hru_windspeed_sta')
        IF ( getparam_int(MODNAME, 'hru_humidity_sta', Nhru, Hru_humidity_sta)/=0 ) &
             CALL read_error(2, 'hru_humidity_sta')
