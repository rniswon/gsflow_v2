...
! to each HRU from one or more stations using monthly correction
...
!***********************************************************************
      MODULE PRMS_PRECIP_1STA_LAPS
        ...
        character(len=*), parameter :: Version_precip = '2025-02-12'
...
! Get parameters
      ELSEIF ( Process_flag==INIT ) THEN
        ...
        IF ( Precip_flag==precip_1sta_module ) THEN
          IF ( getparam_real(MODNAME, 'rain_adj', Nhru*Nmonths, Rain_adj_lapse)/=0 ) CALL read_error(2, 'rain_adj')
          IF ( getparam_real(MODNAME, 'snow_adj', Nhru*Nmonths, Snow_adj_lapse)/=0 ) CALL read_error(2, 'snow_adj')
        ELSE
