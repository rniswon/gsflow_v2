...
!    Ag_gravity_flag = OFF
    ...
    Gwr_swale_flag = OFF
    i = i + 1
    Control_parameter_data(i) % name = 'gw2dprst_swale_flag'
    gw2dprst_swale_flag = OFF
    i = i + 1
...
!    Control_parameter_data(i) % allocate_flag = 1 ! need to allocate
 ...
 !   Control_parameter_data(i) % data_type = REAL_TYPE
    ...
    Executable_desc = 'GSFLOW Version 2.4.1'
 ...
 9001 format('    gsflow version: ', A, /, '    Date created: ', I4, 2('/', I2.2), I3.2, ':', I2.2,/)
    ...
    IF ( MODSIM_flag == OFF ) THEN
      ...
      if ( num_words_command_line == 1 ) &  ! no arguments after executable name
  ...
  module subroutine get_control_arguments()
    use PRMS_CONSTANTS, only: DEBUG_less, ERROR_control, OFF
