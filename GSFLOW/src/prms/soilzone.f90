...
!   Local Variables
      ...
      character(len=*), parameter :: Version_soilzone = '2023-02-23'
      INTEGER, SAVE :: DBGUNT
...
!***********************************************************************
      ...
      USE PRMS_CONSTANTS, ONLY: ERROR_dim, ACTIVE, OFF, DEBUG_WB, &
     &    CASCADE_OFF, MONTHS_PER_YEAR
      ...
      USE PRMS_MODULE, ONLY: Nhru, Nsegment, Nlake, Nhrucell, Print_debug, Cascade_flag, GSFLOW_flag, Iter_aet_flag
      USE PRMS_SOILZONE
...
!***********************************************************************
      ...
      USE PRMS_MODULE, ONLY: Nhru, Nssr, Nlake, Nhrucell, Dprst_flag, Init_vars_from_file, Cascade_flag, &
     ...
     &    Hru_impervstor, Dprst_stor_hru, Hru_intcpstor, Snowcov_area, Gravity_stor_res
...
! WARNING changing parameter values as invalid for these HRU types, need to be 0 for computations below
        ...
        IF ( perv_area>0.0 ) THEN
          IF ( Soil_moist_max(i)>0.0 ) Basin_cpr_stor_frac = Basin_cpr_stor_frac + &
     &         DBLE( Soil_moist(i)/Soil_moist_max(i)*perv_area )
          ...
          IF ( Soil_zone_max(i) > 0.0 ) Basin_sz_stor_frac = Basin_sz_stor_frac + &
     &         DBLE( Soil_moist_tot(i)/Soil_zone_max(i)*hruarea )   !RGN added to avoid divide by zero 1/20/2022
...
! ***************************************
      ...
      IF ( Pref_flag==ACTIVE ) THEN
        ...
        Basin_pref_flow_max_in = Basin_pref_flow_max_in*Basin_area_inv
      ...
      ENDIF
      IF ( adjust_hortonian==ACTIVE ) THEN ! need for CFGI, srunoff water balance will be incorrect, probably need another variable
...
!***********************************************************************
     ...
     &           Snow_free, Potet_rechr, Potet_lower, Potet, Perv_frac, Soil_saturated, hru_id, ag_perv_flag)
      USE PRMS_CONSTANTS, ONLY: NEARZERO, BARESOIL, SAND, LOAM, CLAY, ACTIVE, OFF, CLOSEZERO
...
!          print *, potet_rechr, potet_lower, avail_potet, (Potet_rechr+Potet_lower)*Perv_frac
        ...
        ELSE
          ...
          IF ( Soil_rechr < CLOSEZERO ) Soil_rechr = 0.0
        ENDIF
...
!***********************************************************************
      ...
      USE PRMS_CONSTANTS, ONLY: ERROR_soilzone, CLOSEZERO !, NEARZERO
