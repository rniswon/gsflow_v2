...
!***********************************************************************
     ...
     &    PRMS_land_iteration_flag, Ag_package, AG_flag, PRMS_land_interation_flag
      USE PRMS_SOILZONE
...
!??? figure out what to save in restart file ???
      IF ( PRMS_land_interation_flag==ACTIVE ) THEN
...
!***********************************************************************
     ...
     &    Soilzone_aet_flag, Nowyear, Nowmonth, Nowday
      USE PRMS_SOILZONE
...
!****** add soil excess (Dunnian flow) to infiltration
        ...
        IF ( Ag_package==ACTIVE ) THEN
          IF ( Hru_ag_irr(i)>0.0 ) THEN
            ag_water_maxin = Hru_ag_irr(i)/perv_frac ! Hru_ag_irr is in inches
          ENDIF
        ENDIF
...
!     &             Ag_irrigation_add(i) = Ag_irrigation_add(i) + ag_avail_targetAET
          ...
          ELSEIF ( ag_AETtarget<Hru_actet(i) ) THEN
     ...
     &                           Hru_actet(i), ' PET_external: ', PET_external(i), ' AET_external: ', AET_external(i)
          ...
          ELSE
            print *, i, AET_external(i), Hru_actet(i), ag_avail_targetAET, Ag_irrigation_add(i) 
          ENDIF
          Unused_ag_et(i) = ag_AETtarget - ag_avail_targetAET
...
!        IF ( Hru_actet(i)>0.0 ) Snowevap_aet_frac(i) = Snow_evap(i)/Hru_actet(i)
      ...
      IF ( Iter_aet_flag==OFF ) keep_iterating = OFF
