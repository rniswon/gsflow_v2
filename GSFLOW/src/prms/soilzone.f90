...
!   Local Variables
      ...
      character(len=*), parameter :: Version_soilzone = '2021-05-07'
...
! Agriculture variables and parameters
     ...
     &     'Storage in the lower zone of the agriculture reservoir that is only available for transpiration for each HRU', &
     &     'inches', Ag_soil_lower)
        ...
        IF ( GSFLOW_flag==ACTIVE .OR. MODEL==PRMS_AG .OR. Model==DOCUMENTATION ) THEN
          ...
          CALL declvar_real(MODNAME, 'ag_gvr2sm', 'nhru', Nhru, & ! allocated above
