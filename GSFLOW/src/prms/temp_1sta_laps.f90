...
!***********************************************************************
      MODULE PRMS_TEMP_1STA_LAPS
        ...
        character(len=*), parameter :: Version_temp = '2025-01-11'
...
! 3 degC/ 1000 ft is adiabatic, or 9.8 degC/ 1000 m, or 5.4 degF/ 1000 ft, or 17.64 degF /1000 m
     ...
     &       'none')/=0 ) CALL read_error(1, 'max_missing')
      ...
      ELSEIF ( Process_flag==INIT ) THEN
        ...
        IF ( Temp_flag==temp_1sta_module ) THEN
          IF ( getparam_real(MODNAME, 'tmin_lapse', Nhru*Nmonths, Tmin_lapse)/=0 ) CALL read_error(2, 'tmin_lapse')
          IF ( getparam_real(MODNAME, 'tmax_lapse', Nhru*Nmonths, Tmax_lapse)/=0 ) CALL read_error(2, 'tmax_lapse')
        ELSEIF ( Temp_flag==temp_laps_module ) THEN
        ...
        DO i = 1, Active_hrus
          j = Hru_route_order(i)
          IF ( Hru_tsta(j) == 0 ) THEN
            PRINT *, 'ERROR: hru_tsta cannot be 0 for HRU', j, ' in temp_1sta_laps'
            ierr = 1
          ENDIF
        ENDDO
        IF ( ierr==1 ) THEN
